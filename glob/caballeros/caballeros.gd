...
const SETTING_PHYSICS := "physics/2d/default_gravity"
const GROUP_PLAYER := "player"
const ANIM_MORIR := "morir"
const ANIM_ATACAR := "atacar"
const VAR_IS_ATTACKING := "is_attacking"
const PROP_MONITORING := "monitoring"

@export var speed: float = 100.0
@onready var gravity: float = ProjectSettings.get_setting(SETTING_PHYSICS)

var sentido = 1
var atacando = false
var puede_atacar = true

@onready var jugador = get_tree().get_first_node_in_group(GROUP_JUGADORES)
@onready var cuerpo = $Cuerpo
@onready var sprite = $Cuerpo/ani_ene_dyn
...
func morir_enemigo():
    ...
    $Cuerpo/Area2D.set_deferred(PROP_MONITORING, false)
